    ...
    return (
      ...
      <div className="favorites">
          ...
          {this.props.listMovies.map((item) => {
            return (
              <li className="favorites__list--item" key={item.imdbID}>
                ...
                <button className="favorites__list--delete" onClick={() => this.props.removeMovie(item.imdbID)}>🗑️</button>
              </li>
            );
          })}
        </ul>
        ...
        <button type="button" onClick={() => this.handleSaveList()}
                className={`favorites__save ${this.state.linkActive ? "link__none" : null} `} disabled={!this.state.title||this.props.listMovies.length===0}>
          Save
        </button>
        </div>
      </div>
